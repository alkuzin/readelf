...
/// shared object files to hold the right informa- tion for a process’s program
/// image. Relocation entries are these data.
#[repr(C, packed)]
pub struct Elf32_Rel {
    /// This member gives the location at which to apply the relocation action.
    /// For a relocatable file, the value is the byte offset from the
    /// beginning of the section to the storage unit affected
    /// by the relocation. For an executable file or a shared object, the value
    ...
    /// example, a call instruction’s relocation entry would hold the
...
/// - `S` - This means the value of the symbol whose index resides in the
///   relocation entry.
...
/// table entry and additionally instructs the link editor to build a procedure
/// linkage table.
pub const R_386_PLT32: u32 = 4;
/// No relocation is needed.
pub const R_386_COPY: u32 = 5;
/// This relocation type is used to set a global offset table entry to the
/// address of the specified symbol. It allows one to determine the
/// correspondence between symbols and global offset table entries.
pub const R_386_GLOB_DAT: u32 = 6;
/// This relocation type is created for dynamic linking. Its offset member gives
/// the location of a procedure linkage table entry. The dynamic linker modifies
/// the procedure linkage table entry to transfer control to the designated
/// symbol’s address.
...
/// which the shared object was loaded to the relative address. Relocation
...
/// the address of the global offset table. It additionally instructs the link
...
/// global offset table in its calculation. The symbol referenced in this
/// relocation normally is _GLOBAL_OFFSET_TABLE_, which additionally instructs
