...
use crate::elf::{ Elf32_Addr, Elf32_Half, Elf32_Off, Elf32_Sword, Elf32_Word };
...
/// for dynamic linking. Consequently, an object file may also contain a SHT_DYNSYM section,
...
/// Elf32_Rel for the 32-bit class of object files. An object file may have multiple relocation sections.
...
/// The section is allocated in memory and is writable, as indicated by the presence of the
/// SHF_ALLOC and SHF_WRITE flags.
///
/// # Parameters
/// - `section` - given ELF section header struct.
/// - `name`    - given section name.
pub fn is_data_section(section: &Elf32_Shdr, name: &str) -> bool {
    section.sh_type == SHT_PROGBITS && (section.sh_flags
        & (SHF_ALLOC | SHF_WRITE)) == (SHF_ALLOC | SHF_WRITE) && name == ".data"
}

/// Check if a section is .data1.
///
/// - `.data1` - This section holds initialized data that contribute to the program’s memory image.
/// Similar to `.data`, it is allocated in memory and is writable, as indicated by the presence of
/// the SHF_ALLOC and SHF_WRITE flags.
///
/// # Parameters
/// - `section` - given ELF section header struct.
/// - `name`    - given section name.
...
pub fn is_data1_section(section: &Elf32_Shdr, name: &str) -> bool {
    section.sh_type == SHT_PROGBITS && (section.sh_flags
        & (SHF_ALLOC | SHF_WRITE)) == (SHF_ALLOC | SHF_WRITE) && name == ".data1"
}

/// Check if a section is .debug.
///
/// - `.debug` - This section holds information for symbolic debugging. The contents are unspecified,
/// and it is typically used by debuggers to provide information about the source code and its
...
pub fn is_debug_section(section: &Elf32_Shdr, name: &str) -> bool {
    section.sh_type == SHT_PROGBITS && section.sh_flags == 0
    && name == ".debug"
}
...
/// include the SHF_ALLOC bit. Whether the SHF_WRITE bit is set is processor-specific. It is used
/// by the dynamic linker to manage shared libraries and dynamic symbols.
///
/// # Parameters
/// - `section` - given ELF section header struct.
/// - `name`    - given section name.
...
/// that represent the names associated with symbol table entries. It is essential for the dynamic
/// linker to resolve symbol names at runtime.
///
/// # Parameters
/// - `section` - given ELF section header struct.
/// - `name`    - given section name.
...
pub fn is_dynstr_section(section: &Elf32_Shdr, name: &str) -> bool {
    section.sh_type == SHT_STRTAB && (section.sh_flags
        & SHF_ALLOC) == SHF_ALLOC && name == ".dynstr"
}

/// Check if a section is .dynsym.
///
/// - `.dynsym` - This section holds the dynamic linking symbol table, which contains entries for
/// symbols that are used during dynamic linking. It is crucial for the dynamic linker to resolve
/// symbols at runtime.
///
/// # Parameters
/// - `section` - given ELF section header struct.
...
pub fn is_dynsym_section(section: &Elf32_Shdr, name: &str) -> bool {
    section.sh_type == SHT_DYNSYM && (section.sh_flags
        & SHF_ALLOC) == SHF_ALLOC && name == ".dynsym"
}

/// Check if a section is .fini.
///
/// - `.fini` - This section holds executable instructions that contribute to the process termination
/// code. When a program exits normally, the system arranges to execute the code in this section.
///
/// # Parameters
/// - `section` - given ELF section header struct.
/// - `name`    - given section name.
...
pub fn is_fini_section(section: &Elf32_Shdr, name: &str) -> bool {
    section.sh_type == SHT_PROGBITS && (section.sh_flags & (SHF_ALLOC
        | SHF_EXECINSTR)) == (SHF_ALLOC | SHF_EXECINSTR) && name == ".fini"
}

/// Check if a section is .got.
///
/// - `.got` - This section holds the global offset table, which is used for dynamic linking to
/// resolve addresses of global variables and functions. It is essential for the correct execution
...
///
/// - `.hash` - This section holds a symbol hash table, which is used by the dynamic linker to
/// quickly resolve symbols. It provides a way to efficiently look up symbols during dynamic linking.
///
/// # Parameters
/// - `section` - given ELF section header struct.
/// - `name`    - given section name.
...
/// before calling the main program entry point (commonly `main` for C programs).
///
/// # Parameters
/// - `section` - given ELF section header struct.
/// - `name`    - given section name.
...
/// correspondence between the source program and the machine code. The contents are unspecified.
///
/// # Parameters
/// - `section` - given ELF section header struct.
/// - `name`    - given section name.
...
///
/// - `.note` - This section holds information in a format that is described in the "Note Section"
/// in the ELF specification. It is often used for storing metadata about the file.
///
/// # Parameters
/// - `section` - given ELF section header struct.
/// - `name`    - given section name.
...
///
/// - `.plt` - This section holds the procedure linkage table, which is used for dynamic linking.
/// It allows for the resolution of function addresses at runtime, enabling calls to shared library
/// functions.
///
/// # Parameters
/// - `section` - given ELF section header struct.
...
///
/// - `.rodata` - This section holds read-only data that typically contributes to a non-writable
/// segment in the process image. It is used for constants and string literals that should not be
...
pub fn is_rodata_section(section: &Elf32_Shdr, name: &str) -> bool {
    section.sh_type == SHT_PROGBITS && (section.sh_flags
        & SHF_ALLOC) == SHF_ALLOC && name == ".rodata"
}

/// Check if a section is .rodata1.
///
/// - `.rodata1` - This section holds additional read-only data that typically contributes to a
/// non-writable segment in the process image. Similar to `.rodata`, it is used for constants and
/// string literals that should not be modified during execution.
///
/// # Parameters
/// - `section` - given ELF section header struct.
/// - `name`    - given section name.
...
pub fn is_rodata1_section(section: &Elf32_Shdr, name: &str) -> bool {
    section.sh_type == SHT_PROGBITS && (section.sh_flags
        & SHF_ALLOC) == SHF_ALLOC && name == ".rodata1"
}

/// Check if a section is .shstrtab.
///
/// - `.shstrtab` - This section holds section names, which are used to identify the various sections
/// in the ELF file. It is essential for the proper interpretation of the section headers.
///
/// # Parameters
/// - `section` - given ELF section header struct.
/// - `name`    - given section name.
...
/// associated with symbol table entries. If the file has a loadable segment that includes the
/// symbol string table, the section’s attributes will include the SHF_ALLOC bit; otherwise, that
/// bit will be off.
///
/// # Parameters
/// - `section` - given ELF section header struct.
...
///
/// - `.symtab` - This section holds a symbol table, which contains information about the symbols
/// used in the program. It provides a mapping between symbolic names and their corresponding
/// addresses or values. If the file has a loadable segment that includes the symbol table, the
...
///
/// - `.text` - This section holds the "text," or executable instructions, of a program. It
/// contains the compiled code that the CPU executes. The section is typically marked as
/// executable and may also be marked as readable, but not writable.
///
/// # Parameters
/// - `section` - given ELF section header struct.
/// - `name`    - given section name.
...
pub fn is_text_section(section: &Elf32_Shdr, name: &str) -> bool {
    section.sh_type == SHT_PROGBITS && (section.sh_flags & (SHF_ALLOC
        | SHF_EXECINSTR)) == (SHF_ALLOC | SHF_EXECINSTR) && name == ".text"
}