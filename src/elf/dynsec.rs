...
/// a needed library. The offset is an index into the table recorded in the `DT_STRTAB` entry.
/// The dynamic array may contain multiple entries with this type. These entries’ relative order
/// is significant, though their relation to entries of other types is not.
pub const DT_NEEDED: u32 = 1;
/// This element holds the total size, in bytes, of the relocation entries associated with the
/// procedure linkage table. If an entry of type `DT_JMPREL` is present, a `DT_PLTRELSZ` must
/// accompany it.
pub const DT_PLTRELSZ: u32 = 2;
/// This element holds an address associated with the procedure linkage table and/or the global
/// offset table. See this section in the processor supplement for details.
pub const DT_PLTGOT: u32 = 3;
/// This element holds the address of the symbol hash table, described in "Hash Table." This
/// hash table refers to the symbol table referenced by the `DT_SYMTAB` element.
...
/// This element holds the address of the symbol table, described in Part 1, with `Elf32_Sym`
...
/// or shared object file, the link editor concatenates those sections to form a single table.
/// If this element is present, the dynamic structure must also have `DT_RELASZ` and `DT_RELAENT`
/// elements. When relocation is "mandatory" for a file, either `DT_RELA` or `DT_REL` may occur
...
/// This element holds the string table offset of a null-terminated string, giving the name of
/// the shared object. The offset is an index into the table recorded in the `DT_STRTAB` entry.
...
/// resolution algorithm for references within the library. Instead of starting a symbol search
/// with the executable file, the dynamic linker starts from the shared object itself. If the
/// shared object fails to supply the referenced symbol, the dynamic linker then searches the
...
/// The `d_val` member holds `DT_REL` or `DT_RELA`, as appropriate. All relocations in a
/// procedure linkage table must use the same relocation.
pub const DT_PLTREL: u32 = 20;
/// This member is used for debugging. Its contents are not specified for the ABI; programs
...
/// table. If this member is present, one or more relocation entries might request modifications
/// to a non-writable segment, and the dynamic linker can prepare accordingly.
pub const DT_TEXTREL: u32 = 22;
/// If present, this entry’s `d_ptr` member holds the address of relocation entries associated
